...
def printCopyright():
    ...
    prCyan("                                      ")
    prCyan(" NovinCeram Delta Robot Simulator and Gcode Sender")
    prCyan(" Ver Code : #3")
    prCyan(" Sep, 2019")
    prCyan(" Optimized Final Version With Serial")
...
for i in range(38):
    ...
    prCyan("______________________________________")
    prCyan("                                      ")
    prCyan("Code & Algorithm by Amirmohammad Zarif")
    prCyan("                                      ")
    prCyan("_______________________________________")
    prCyan("                                      ")
    DashedLines += "_"
...
def line2Calc(startX, startY, startZ, angle, length, le, width):
    ...
    endY = widthY+ startY
    endZ = widthZ + startZ
    # prLightGray('lineCalc #2 : ', [endX,endY,endZ])
    # prCyan("Check : ", [widthX,widthY,widthZ])
...
theta2 = 90
theta3 = 90
Machine.write("X," + str(theta1) + "\n")
Machine.write("Y," + str(theta2) + "\n")
Machine.write("Z," + str(theta3) + "\n")
for i in range(-42,98):
    if i % 10 == 0: 
        theta1 = i + 90
        theta2 = i + 90
        theta3 = i + 90
        prRed("Theta 1 : " + str(theta1))
        prRed("Theta 2 : " + str(theta2))
        prRed("Theta 3 : " + str(theta3))
        Machine.write("X," + str(theta1) + "\n")
        Machine.write("Y," + str(theta2) + "\n")
        Machine.write("Z," + str(theta3) + "\n")
        ax.set_aspect('equal')
        ...
        ax.set_zlabel('Z')
        ########
        prYellow("______________________________________________________________________")
        prYellow(">> Step : " + str(i))
        ########
        ax.cla()
        ########
        ...
        art3d.pathpatch_2d_to_3d(p, z=bp, zdir="z")
        ########
        lineouts1 = lineCalc(point1posX,point1posY,point1posZ,i,rf)

        line2Helper = lineCalc(point2posX,point2posY,point2posZ,i,rf)
        lineouts2 = line2Calc(point2posX,point2posY,point2posZ,i,rf,120,line2Helper[0] - point2posX)

        line3Helper = lineCalc(point3posX,point3posY,point3posZ,i,rf)
        lineouts3 = line2Calc(point3posX,point3posY,point3posZ,i,rf,240,line3Helper[0] - point3posX)
        ########
        ...
        for i in range(3,10):
            ...
            prYellow("Line #" + str(i) + " : " + str([VecEnd_x[i],VecEnd_y[i],VecEnd_z[i]]))
            ax.plot([VecStart_x[i], VecEnd_x[i]], [VecStart_y[i],VecEnd_y[i]],zs=[VecStart_z[i],VecEnd_z[i]],color="g",marker="o")
